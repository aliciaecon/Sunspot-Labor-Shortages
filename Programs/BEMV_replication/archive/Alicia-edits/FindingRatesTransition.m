...
function [ qnew, phinew, pnew, u, V_agg ] = ...
    FindingRatesTransition( q, phi, g_znmat, v_znmat, Gn_znmat,...
    SUntrunc_znmat, MF, ME, NumGrids, Params, ExogParams )
...
g_znmat = reshape(g_znmat,Nz,Nn); % Just in case vector is loaded
...
pi0Trunc_znmat              = pi0_znmat / sum(sum(pi0_znmat .* dzdn_znmat)) ; % Ensure integration to 1 
    ...
    % Aggregate vacancies
Gn_znmat            = reshape(Gn_znmat,Nz,Nn); % Just in case vector is loaded
v_znmat             = reshape(v_znmat,Nz,Nn); % Just in case vector is loaded
...
V_agg               = MF * ( sum(sum( v_znmat .* g_znmat .* dzdn_znmat)) + ME * sum(sum(VacEnt_znmat)) ) ; % Aggregate vacancies, recall exitrate=entryrate
