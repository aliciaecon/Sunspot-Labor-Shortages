...
OVERVIEW:
_________________________________________________________________________________________
README file for 'Firm and Worker Dynamics in a Frictional Labor Market' Replication Package
...
--- The only file in which the user has to input a correct pathway is:
	--- ~/3_Empirics/2_BDS_Statistics_by_age_size/0_Run_all_INPUT_USER_PATH_HERE.do
--- There are three main run files, described below:
	-1- BDS empirics:
		~/3_Empirics/2_BDS_Statistics_by_age_size/0_Run_all_INPUT_USER_PATH_HERE.do
	-2- Cross-MSA empirics:
		~/3_Empirics/1_Cross_MSA_Entry_EE/src/MAIN.do
			*** This should be proceeded by the file that downloads and unzips required data:
				~/3_Empirics/1_Cross_MSA_Entry_EE/src/1_metro_download.sh
				but the replication package comes with this data already downloaded and unzipped
	-3- All MATLAB based figures
		~/2_MATLAB_code/RUN_make_all.m
_________________________________________________________________________________________
~/1_Draft
_________________________________________________________________________________________
