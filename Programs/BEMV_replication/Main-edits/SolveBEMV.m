...
NumGrids    = Grids( Numerical, Params, options ) ;                                      % define grids and useful objects from them     -> (Nz,Nn,z,n,dz_z,dn_n,dzdn_zn,dzdn_znmat,...)
Dmatrices   = MatricesDerivatives( NumGrids, options );                                  % numerical approximation to derivatives        -> (Dn,Dz,Dzz) [forward and backward versions]
...
if options.Hopenhayn == 0
    [ g_znmat, u, p, q, phi, Gn_znmat, ~, Gv_znmat, S_znmat, Sn_znmat, v_znmat, exitrate ] ...
        = InitialGuess( ExogParams, NumGrids, Params, Zmatrices, PerPeriod, options ) ;
...
while error > tol && it < maxiter % In the paper, each of these iterations is "t"
    ...
    %______________________________________________________________________
        ...
        % Setup
    [ Gn_znmat, Ghatn_znmat, inds, ~ ] = Cdf_Gn( Sn_znmat, g_znmat, NumGrids, options ) ; % CDF weighted by employment
