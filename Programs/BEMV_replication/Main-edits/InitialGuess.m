...
function [ g_znmat, u, p, q, phi, Gn_znmat, Ghatn_znmat, Gv_znmat, S_znmat, Sn_znmat, v_znmat, exitrate ]...
    = InitialGuess( ExogParams, NumGrids, Params, MatExog, PerPeriod, options )
%%  LOAD OBJECTS
...
n_zn        = NumGrids.n_zn;        % Replica of the n grid as vector
...
for i = 1:N_end % Correct matrix for non-degeneracy
    ifixloc         = ifixn(i);
    g0_zn(ifixloc)  = 0.1;
    if options.Z == 0
...
[ Gn_znmat, Ghatn_znmat, inds, inds_inv ] = Cdf_Gn( Sn_znmat, g_znmat, NumGrids, options ) ; % Employment-weighted marginal surplus distribution
exitrate = 0 ;
Gv_znmat = Cdf_Gv( inds, inds_inv, v_znmat, g_znmat, S_znmat, exitrate, q, Gn_znmat, phi, NumGrids, options ) ; % Vacancy-weighted marginal surplus distribution
