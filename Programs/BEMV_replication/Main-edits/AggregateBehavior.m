...
[ GnNew_znmat, Ghatn_znmat, inds, inds_inv ]= Cdf_Gn( Sn_znmat, g_znmat, NumGrids, options ) ;
error_Gn                                    = max(max( abs(GnNew_znmat - Gn_znmat) )) ;
Gn_znmat                                    = GnNew_znmat ;

    % STEP IIb - Update the vacancy-weighted distribution
GvNew_znmat = Cdf_Gv( inds, inds_inv, v_znmat, g_znmat, SUntrunc_znmat, exitrate,...
    q, Gn_znmat, phi, NumGrids, options ) ;
...
[ error_f, q, phi, p, u ] = ...
    FindingRates( q, phi, p, g_znmat, v_znmat, exitrate, Gn_znmat, SUntrunc_znmat, NumGrids,...
    Params, ExogParams, options ) ;
